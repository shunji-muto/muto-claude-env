...
- **`routine:ready` ラベルの付与を試みる。付与前に必ずラベルの存在を確認する:**
  - `mcp__github__list_labels` 相当のツールでラベル一覧を取得する。
  - `routine:ready` が存在する場合 → ラベルを付与して起票する。
  - 存在しない場合 → ラベルなしで起票し、Issue 本文冒頭に「`routine:ready` ラベルが未作成のため後で手動付与してください」という警告コメントを追記する。ユーザーにも同じ警告を表示する。
